...
std::optional<std::array<uint8_t, 32>> getEd25519PubKey(std::string pubkey) {
...
bool SignatureUtil::verifySignature(std::string_view bytecode, std::string pubkey,
                                    std::string &message) {
  const auto ed25519_pubkey = getEd25519PubKey(pubkey);
  if (ed25519_pubkey) {
    /*
    ...
    if (!ED25519_verify(hash, sizeof(hash), signature, ed25519_pubkey.value().data())) {
